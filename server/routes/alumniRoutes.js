...
router.get("/", async (req, res) => {
  try {
    ...
    let query =
      "SELECT id, name, branch, passout_year, pg_university, pg_course, pg_country, pg_city, current_company, designation, linkedin_url, willing_to_mentor, areas_of_help, photo_url FROM alumni WHERE alumni_status = 'active'";
    const params = [];
    ...
    if (search) {
      query +=
        " AND (name LIKE ? OR current_company LIKE ? OR pg_university LIKE ? OR pg_course LIKE ?)";
...
router.get("/:id", async (req, res) => {
  try {
    const [alumni] = await pool.query(
      "SELECT id, name, branch, passout_year, pg_university, pg_course, pg_country, pg_city, current_company, designation, linkedin_url, willing_to_mentor, areas_of_help, photo_url FROM alumni WHERE id = ?",
      [req.params.id],
    );
    if (alumni.length === 0)
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Alumni created successfully", id: result.insertId });
  } catch (error) {
    console.error(error);
    res
      .status(500)
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE alumni SET ${setClause} WHERE id = ?`, [
      ...values,
      req.params.id,
    ]);
...
router.post("/:id/connect", async (req, res) => {
  try {
    const [alumniRows] = await pool.query("SELECT * FROM alumni WHERE id = ?", [
      req.params.id,
    ]);
    ...
    if (!studentName || !studentEmail || !message) {
      return res
        .status(400)
        .json({ message: "Student Name, Email, and Message are required" });
    }
    ...
    if (sent) {
      res.json({
        message: `Mentorship request sent to ${alumnus.name} successfully!`,
      });
    } else {
