...
function initMailer() {
    const user = process.env.SMTP_GMAIL_USER || 'shravaniraut2324@ternaengg.ac.in';
    ...
    if (pass) {
        ...
        console.log('SMTP App Password not set in .env. Email service will run in simulation mode (logging emails to console).');
    }
}
 ...
 */
async function sendSubmissionConfirmation({ studentEmail, studentName, tu4fId, department, status, driveFolderUrl }) {
    const subject = `[HEC] Application Received — ${studentName} (${tu4fId})`;
    const html = `
 ...
 */
async function sendDocumentReminder({ studentEmail, studentName, tu4fId, status, notes }) {
    ...
    if (transporter && studentEmail) {
        try {
            ...
            console.error(`Failed to send reminder email to ${studentEmail}:`, err.message);
            return false;
        }
    } else {
        console.log(`[SIMULATION REMINDER] To: ${studentEmail} | Subject: ${subject}`);
        return true;
    }
}
 ...
 */
async function sendAlumniConnectRequest({ alumniEmail, alumniName, studentName, studentEmail, studentPhone, studentBranch, targetCountry, message }) {
    ...
    if (transporter && alumniEmail) {
        try {
            ...
            console.error(`Failed to send connect request to ${alumniEmail}:`, err.message);
            return false;
        }
    } else {
        console.log(`[SIMULATION ALUMNI CONNECT] To: ${alumniEmail} | Student: ${studentName} <${studentEmail}>`);
        return true;
    }
}
