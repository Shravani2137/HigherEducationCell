...
app.get("/", (req, res) => {
  res.json({ service: "Higher Education Cell API", status: "ok" });
});
...
const startServer = async () => {
  try {
    ...
    if (!process.env.ADMIN_PASSWORD || !process.env.JWT_SECRET) {
      throw new Error(
        "ADMIN_PASSWORD and JWT_SECRET must be configured in .env",
      );
    }
    const plainAdminPass = process.env.ADMIN_PASSWORD;
