...
import { FiSearch, FiLinkedin, FiMail, FiBook, FiBriefcase, FiMessageSquare, FiX, FiSend } from 'react-icons/fi';
...
const AlumniDirectory = () => {
  ...
  useEffect(() => {
    fetchAlumni();
  }, [filters.country, filters.branch]);
  ...
  const handleConnectSubmit = async (e) => {
    ...
    try {
      setSending(true);
      await connectWithAlumni(selectedAlumnus.id || selectedAlumnus._id, connectForm);
    ...
    <div className="alumni-page">
      <div className="alumni-header">
        ...
        <p className="text-text-secondary max-w-2xl mx-auto">
          Connect with TEC alumni studying or working globally. Reach out for guidance on university selection, application processes, and career advice.
        </p>
      </div>
      ...
      <div className="alumni-layout">
        ...
        <main className="alumni-grid-container">
          {loading ? (
            <div className="text-center py-12 text-text-secondary">Loading alumni directory...</div>
          ) : alumni.length === 0 ? (
            ...
            <div className="alumni-grid">
              {alumni.map(person => (
                <div key={person.id || person._id} className="card alumni-card flex flex-col justify-between">
                  <div>
                    <div className="alumni-card-header">
                      <div>
                        <h3 className="alumni-name">{person.name}</h3>
                        <div className="text-sm text-text-secondary">
                          {person.branch} • Class of {person.passout_year || person.passoutYear}
                        </div>
                      </div>
                      <span className="alumni-batch">{person.pg_country || person.country}</span>
                    </div>
                    ...
                    <div className="alumni-course">{person.pg_course || person.course}</div>

                    {(person.current_company || person.currentCompany) && (
                      <div className="alumni-work">
                        <div className="flex items-center gap-2 mb-1">
                          <FiBriefcase className="text-accent-teal" />
                          <span className="font-medium">{person.designation || person.currentDesignation}</span>
                        </div>
                        <div className="text-sm text-text-secondary pl-6">{person.current_company || person.currentCompany}</div>
                      </div>
                    )}
                  ...
                  <div className="alumni-links border-t border-border-color pt-3 mt-4 flex items-center justify-between">
                    <div className="flex gap-2">
                      {person.linkedin_url && (
                        <a href={person.linkedin_url} target="_blank" rel="noreferrer" className="alumni-link" title="LinkedIn">
                          <FiLinkedin />
                        </a>
                      )}
                      {person.email && (
                        <a href={`mailto:${person.email}`} className="alumni-link" title="Email">
                          <FiMail />
                        </a>
                      )}
                    </div>
      ...
      {selectedAlumnus && (
        <div className="modal-backdrop flex items-center justify-center p-4 fixed inset-0 bg-black/70 backdrop-blur-sm z-50">
          <div className="card w-full max-w-lg relative animate-fadeIn">
            ...
            <h3 className="text-xl font-heading mb-1">Connect with {selectedAlumnus.name}</h3>
            <p className="text-sm text-text-secondary mb-6">
              {selectedAlumnus.pg_university || selectedAlumnus.university} • {selectedAlumnus.pg_country || selectedAlumnus.country}
            </p>
            ...
            <form onSubmit={handleConnectSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="form-group">
                  ...
                  <input 
                    ...
                    onChange={(e) => setConnectForm(prev => ({...prev, studentName: e.target.value}))}
                  />
                </div>
                <div className="form-group">
                  ...
                  <input 
                    ...
                    onChange={(e) => setConnectForm(prev => ({...prev, studentEmail: e.target.value}))}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="form-group">
                  ...
                  <select 
                    ...
                    onChange={(e) => setConnectForm(prev => ({...prev, studentBranch: e.target.value}))}
                  >
                    <option value="IT">IT</option>
                    <option value="CS">CS</option>
                    <option value="EXTC">EXTC</option>
                ...
                <div className="form-group">
                  <label className="form-label">Target Country/University</label>
                  <input 
                    type="text" 
                    className="input" 
                    placeholder="e.g. USA / CMU"
                    value={connectForm.targetCountry}
                    onChange={(e) => setConnectForm(prev => ({...prev, targetCountry: e.target.value}))}
                  />
                </div>
              </div>

              <div className="form-group">
                <label className="form-label">Message / Question for Mentorship *</label>
                <textarea 
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" className="btn btn-secondary" onClick={() => setSelectedAlumnus(null)}>
                  Cancel
                </button>
                <button type="submit" className="btn btn-primary flex items-center gap-2" disabled={sending}>
